...
class UserProfileViewSet(viewsets.ModelViewSet):
    ...
    def pic_to_store(self, request, pic, prof_id):
        if request.method == 'PATCH':
            b = blob.Blob(pic)
            b = b.to_text()
            u = UserProfile.objects.get(pk=prof_id)
            u.pic = b
            u.save()
            return None
        else:
    ...
    def blob_into_pic(self, requiest, pic, prof_id):
        up = UserProfile.objects.get(pk=prof_id)
        b = Blob(pic)
        pass
